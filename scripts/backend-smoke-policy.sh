...
# Smoke test for backend with inline policy DSL
# Assumes:
# - Generated output at generated/backend-policy (adjust OUT if needed)
# - Backend server running on localhost:3000 (npm start in generated folder)
# - jq optional; node fallback available

OUT="${OUT:-generated/backend-policy}"
...
echo "[0] Generate backend with policy (optional)"
if [ -d "$OUT" ]; then
  echo "Output dir $OUT already exists, skipping generation."
else
  npx tsx src/cli.ts examples/backend-with-policy.dsl.ts --targets=backend --outDir="$OUT"
fi
...
echo "$CREATE_POST"
POST_ID=$(parse_id "$CREATE_POST" '.data.id' || true)
...
echo "Smoke test (policy) finished."
