...
.irgen-search-icon svg {
  width: 100%;
  height: 100%;
  display: block;
}

.irgen-search input[type="search"] {
  width: min(420px, 100%);
...
.irgen-search input[type="search"]:hover {
  border-color: color-mix(in srgb, var(--accent) 35%, var(--border));
}

.irgen-search input[type="search"]:focus {
  outline: none;
  border-color: color-mix(in srgb, var(--accent) 60%, var(--border));
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--accent) 20%, transparent);
}

.irgen-search-results {
  position: absolute;
...
html[data-irgen-sidebar="collapsed"] .irgen-layout-grid {
  grid-template-columns: 1fr;
}

html[data-irgen-sidebar="collapsed"] .irgen-sidebar {
  display: none;
}
...
.irgen-icon-button:hover {
  border-color: color-mix(in srgb, var(--accent) 30%, var(--border));
  box-shadow: 0 6px 16px rgba(15, 23, 42, 0.12);
  transform: translateY(-1px);
}

.irgen-icon-button:active {
...
.irgen-heading:hover .irgen-heading-copy,
.irgen-heading-copy:focus-visible {
  opacity: 1;
  transform: translateY(0);
  color: var(--fg);
}

.irgen-heading-copy.is-copied {
  color: var(--accent);
}
...
.irgen-marketing .irgen-list li {
  position: relative;
  padding-left: 1.75rem;
  color: var(--fg);
}

.irgen-marketing .irgen-list li::before {
  content: "";
  width: 0.75rem;
  height: 0.75rem;
  border-radius: 999px;
...
.irgen-marketing .irgen-list li span {
  color: var(--muted);
  margin-left: 0.35rem;
}
...
code:not(pre code) {
  background: color-mix(in srgb, var(--code-bg) 85%, var(--bg));
...
export async function emitStaticSiteCss(
  outDir: string,
  options?: { accentColor?: string },
): Promise<void> {
  const assetsDir = path.join(outDir, "assets");
  await fs.mkdir(assetsDir, { recursive: true });
  const overrides = options?.accentColor
    ? `\n:root { --accent: ${options.accentColor}; }\n`
    : "\n";
  await fs.writeFile(path.join(assetsDir, "style.css"), `${BASE_CSS}${overrides}`, "utf-8");
}

export async function appendCustomCss(outDir: string, customCssPath: string): Promise<void> {
  const assetsDir = path.join(outDir, "assets");
  const cssPath = path.join(assetsDir, "style.css");
  const absCustomPath = path.isAbsolute(customCssPath)
    ? customCssPath
    : path.resolve(process.cwd(), customCssPath);

  try {
    const content = await fs.readFile(absCustomPath, "utf-8");
    const extra = `\n/* Custom CSS */\n${content}\n`;
    await fs.appendFile(cssPath, extra, "utf-8");
  } catch (err) {
    console.warn(`[static-site emitter] Failed to read custom CSS: ${absCustomPath}`);
  }
}
