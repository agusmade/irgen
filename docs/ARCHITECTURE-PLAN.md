# Architecture Plan — Unified Pipeline

This document outlines the architecture plan to migrate the project to a unified, extensible pipeline as described in diagram (DSL → DeclIR → DomainIRs → Lowering → TargetIR → Emit). It includes the provided mermaid diagram, explanations of elements, phased implementation plan, acceptance criteria, and estimates.
...
flowchart TB
  ...
  subgraph A[Authoring & Inputs]
    DSL["Developer DSL<br>(JS/TS)<br>app/page/entity/service..."]
    SCHEMA["Schema / Models<br>(DB schema / table/relasi / OpenAPI)<br>optional"]
    ASSETS["Assets/Resources<br>(files, templates? optional)<br>optional"]
  end
  ...
  subgraph B["Build Pipeline (deterministic)"]
    CLI["CLI / Runner<br>(mytool build ...)"]
    RT["DSL Runtime / Evaluator<br>(exec DSL, collect declarations)"]
    DECL["DeclIR (Unified Declarative IR)<br>agnostic, dekat ke DSL"]
    VALID["Validate + Normalize<br>canonical naming rules"]
    MAP["Domain Mapper<br>DeclIR -> DomainIR"]
  end
  ...
  subgraph C["Domain IRs (schema can differ)"]
    FEIR["FrontendIR<br>(routes, pages, forms, tables)"]
    BEIR["BackendIR<br>(entities, ops, services, controllers)"]
    CLIIR["CLIR<br>(commands, flags, workflows)"]
  end
  ...
  subgraph D["Lowering Rules (the 'brain')"]
    LOWER["Lowering Pipeline<br>DomainIR -> TargetIR<br>(conventions, policies)"]
    REACTIR[ReactIR]
  ...
  subgraph E["Emitters (no string templates)"]
    AST["AST Builders<br>(ts-morph / babel AST)"]
    PRINT["Printer/Formatter<br>(prettier/biome optional)"]
    FS["File Emitter<br>(write files, folder layout)"]
  end

  subgraph F["Generated Codebase (real source)"]
    GEN["Project Output<br>(src/*, package.json, routes, services...)"]
    SHARED["Shared Adapters (single point of truth)<br>/lib/id.ts<br>/lib/http.ts<br>/lib/logger.ts<br>..."]
  end
  ...
  subgraph G[Consistency Example: ID generation]
    IR_OP["IR Contract<br>Operation: GENERATE_ID<br>(no uuid/uniqid in IR)"]
    LOWER_ID["Lowering picks implementation<br>(uuid_v4 | uniqid | ...)"]
    WRAP["Shared wrapper<br>newId() in /lib/id.ts"]
    USE["All generated files use:<br>import { newId } from '@/lib/id'<br>const id = newId();"]
  end
