...
## What I implemented
- `src/ir/decl-unified.schema.ts` — zod schema for `DeclUnified` and `validateAndNormalizeDeclUnified()` that performs:
...
- `src/decl/aggregator.ts` — now calls `validateAndNormalizeDeclUnified()` before returning, so aggregator returns a validated, normalized `DeclUnified`.
