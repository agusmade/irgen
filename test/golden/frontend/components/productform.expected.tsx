...
export function ProductForm() {
  ...
  const evalExpr = (expr?: string, fallback?: any) => { try { return expr ? Function("ctx", "with(ctx){ return (" + expr + "); }")(ctx) : fallback; } catch (_) { return fallback; } };
  const validate = () => {
    const n: Record<string,string> = {};
    if (true) {
      const v = id;
      const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === "");
      if (isEmpty) n["id"] = "ID is required";
    }
    if (!n["id"] && undefined !== undefined) {
      const requiredDyn = evalExpr(undefined, false);
      if (requiredDyn) { const v = id; const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === ""); if (isEmpty) n["id"] = "ID is required"; }
    }
    if (true) {
      const v = name;
      const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === "");
    ...
    if (!n["name"] && undefined !== undefined) {
      const requiredDyn = evalExpr(undefined, false);
      if (requiredDyn) { const v = name; const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === ""); if (isEmpty) n["name"] = "Name is required"; }
    }
    if (true) {
      const v = price;
      const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === "");
    ...
    if (!n["price"] && undefined !== undefined) {
      const requiredDyn = evalExpr(undefined, false);
      if (requiredDyn) { const v = price; const isEmpty = Array.isArray(v) ? v.length === 0 : (typeof v === "boolean") ? !v : (!v || v.toString().trim() === ""); if (isEmpty) n["price"] = "Price is required"; }
    }
  ...
  const onSubmit = async (e: any) => { e.preventDefault(); setSubmitSuccess(null); setSubmitError(null); if (!validate()) return;
    ...
    } catch (err: any) { setSubmitError("Submit error"); } finally { setSubmitting(false); }
  };
  return (
